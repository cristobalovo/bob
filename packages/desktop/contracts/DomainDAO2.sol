...
import "https://github.com/OpenZeppelin/openzeppelin-contracts/contracts/payment/escrow/Escrow.sol";
...
contract DomainDAO {
  ...
  /**
   * @dev Withdraws all ETH from domainDAO, recalculates equity for all investors
   */
  function divest() public isInvestor() {
      totalShares.sub(investorShares[msg.sender]);
  ...
  /**
   * @dev Removes investor from all arrays and mappings used for equity
   * @param investorAddress address of investor to be removed
   */
  function removeInvestor(address investorAddress) private {
    address payable shiftedInvestorAddress = investorArray[investorArray.length - 1];
    investorIndex[shiftedInvestorAddress] = investorIndex[investorAddress];

    delete investorIndex[investorAddress];
    delete investorArray[investorArray.length - 1];
    investorArray.length--;

    investorSharesArray[investorIndex[investorAddress]] = investorSharesArray[investorArray.length - 1];
    delete investorSharesArray[investorSharesArray.length - 1];
    investorSharesArray.length--;

    investorShares[investorAddress] = 0;

    investors[investorAddress] = false;
  }
